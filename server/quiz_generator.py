...
def find_notes(topic=None):
    notes = []
    if not os.path.exists(NOTES_DIR): return notes
            ...
            if f.endswith('.md') and not f.startswith('模板_'):
                ...
                for line in content.split('\n'):
                    if line.startswith('topic:') or line.startswith('topics:'):
                        topics = [t.strip().strip('"[]') for t in line.split(':')[1].split(',')]
                        break
                title = f.replace('.md', '')
                for line in content.split('\n'):
                    if line.startswith('# '): title = line[2:].strip(); break
                notes.append({'path': path.replace(NOTES_DIR + os.sep, ''), 'title': title, 'topics': topics, 'content': content, 'file_hash': hashlib.sha256(content.encode()).hexdigest()})
    if topic:
    ...
    notes = [n for n in notes if 'status: ready' in n.get('content', '') or 'status: needs_example' in n.get('content', '')]
    return notes
